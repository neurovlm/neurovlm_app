...
NParray.fromURL = function(url, callback) {
	...
	if (url.constructor == Float32Array){

		var shape = [1, 327684];
		var array = new NParray(dtypeNames['<f4'], shape);
		//array.data = new Float32Array(url);
		array.data = url;

	// 	console.log(array.data);


	// 	json = JSON.stringify(url, null, 2);
	// 	blob = new Blob([json], { type: "application/json" });
	// 	url = URL.createObjectURL(blob);
	// 	a = document.createElement("a");
	// 	a.href = url;
	// 	a.download = "data.json";  // Name of the file to be downloaded
	// 	document.body.appendChild(a);  // Append the link to the body (not visible)
    ...
    xhr.onload = function() {
        ...
        if (magic.slice(1, 6) != 'NUMPY'){
			// Assumes a 1d javascript f32 array of length == fsaverage == 327684
			//   this will produce a buffer of length 1310736
			var shape = [1, 327684];
			var array = new NParray(dtypeNames['<f4'], shape);
			array.data = new Float32Array(url);
