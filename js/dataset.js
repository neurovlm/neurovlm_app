var dataset = (function(module) {
    ...
    module.VertexData = function(json, images) {
		...
		if (this.data[0].constructor == Float32Array){

			//var left = this.data[0].subarray(0, this.split*(163842));
			//var right = this.data[0].subarray(this.split*(163842));
			var left = this.data[0].slice(0, 163842);
			var right = this.data[0].slice(163842);
			...
			// var lattr = new THREE.BufferAttribute(sleft, 1);
			// var rattr = new THREE.BufferAttribute(sright, 1);

			// lattr.needsUpdate = true;
			// rattr.needsUpdate = true;

			var lattr = new THREE.BufferAttribute(left, 1);
			var rattr = new THREE.BufferAttribute(right, 1);

			lattr.needsUpdate = true;
			rattr.needsUpdate = true;
			this.verts.push([lattr, rattr]);
			this.loaded.notify(1);
		} else{
