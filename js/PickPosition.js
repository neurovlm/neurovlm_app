function PickPosition(surf, posdata) {
    ...
    var worker = new Worker(window.URL.createObjectURL(new Blob([document.getElementById('facepick_worker').textContent])));
...
PickPosition.prototype = {
    ...
    pick: function(renderer, scene, x, y, keep) {
    	...
    	if (p) {
            ...
            selected_idx = this.revIndex[p.hemi][p.ptidx];

            const event = new CustomEvent("idxChanged", {
                detail: selected_idx  // Pass the updated value as the event detail
              });

            // Dispatch the event globally (this will be listened by index.js)
